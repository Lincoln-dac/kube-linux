...
EXCLUDE_DIRS = [
    "/app/applogs/gray/sandbox",
    "/app/applogs/gray/retry",
    "/app/applogs/gray/presstest",
    "/app/applogs/traefik",
    "/app/applogs/sandbox",
    "/app/applogs/retry",
    "/app/applogs/presstest",
]
...
class Watcher:
    def __init__(self, configfile, logdir, excludes=None):
    ...
    def __is_excluded(self, path):
        ...
        for base in self.excludes:
            if ap == base or ap.startswith(base + "/"):
                return True
        return False
    ...
    def __event_queue(self, event):
        try:
            ep = getattr(event, "pathname", None) or os.path.join(event.path, event.name)
        except Exception:
            ep = None
        if ep and self.__is_excluded(ep):
            logging.debug("忽略排除目录事件: %s" % ep)
            return
    ...
    def handler(self):
        while True:
            ...
            pathname = getattr(event, "pathname", None) or os.path.join(event.path, event.name)
            if pathname and self.__is_excluded(pathname):
                logging.debug("忽略排除目录事件: %s" % pathname)
                continue
...
if __name__ == "__main__":
    ...
    logging.info("排除目录: %s" % ",".join(excludes))

    watcher = Watcher(args.config, args.dir, excludes=excludes)
    watcher.run_forever()
