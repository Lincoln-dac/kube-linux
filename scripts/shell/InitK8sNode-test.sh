...
if ! [[ " ${allowed_values[*]} " =~ " ${network_zone} " ]]; then
    handle_error "错误：变量${network_zone}无效，只允许输入 test。"
fi
...
EOF
    ...
    wget -O rc.local "${base_url}/k8s/config/rc.local" || handle_error "下载rc.local文件失败"
...
InitFlanneld() {
    ...
    wget -O ssl.tar.gz "${base_url}/k8s/ssl/ssl.tar.gz" || handle_error "下载ssl.tar.gz文件失败"
    ...
    wget -O flanneld "${base_url}/k8s/config/flanneld" || handle_error "下载flanneld配置文件失败"
...
InitDocker() {
    ...
    wget -O config.json "${base_url}/k8s/config/config.json" || handle_error "下载config.json文件失败"
}
...
InitKubelet() {
    ...
    wget -O  kubelet "${base_url}/k8s//config/kubelet" || handle_error "下载kubelet配置文件失败"
    wget -O  kubelet-config.yaml "${base_url}/k8s/config/kubelet-config.yaml" || handle_error "下载kubelet-config.yaml文件失败"
...
InitKubeProxy() {
    echo "初始化kube-proxy"
    cd /app/kubernetes/config/ || handle_error "切换到 /app/kubernetes/config/ 目录失败"
    wget -O proxy "${base_url}/k8s/config/proxy" || handle_error "下载proxy配置文件失败"
    ...
    echo "初始化组件配置文件IP信息完成"
    systemctl daemon-reload && systemctl enable kubelet && systemctl enable kube-proxy && systemctl enable flanneld && systemctl enable docker || handle_error "设置kubelet、kube-proxy、flanneld、docker开机启动失败"
}
...
InitLogAgent() {
    ...
    chown -R root:root /app/filebeat-os/ || handle_error "修改权限失败"
